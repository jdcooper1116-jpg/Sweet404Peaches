...
import { BookType, Plus, Search, Sparkles, Upload } from 'lucide-react';
...
export default function DictionaryPage() {
  ...
  const [gameFilter, setGameFilter] = useState<'ALL' | 'cash3' | 'cash4' | 'mixed'>('ALL');
  ...
  const filteredRows = useMemo(() => {
    return dictionaryRows.filter(item => {
      const termOk =
        ...
        item.numbers.some((num: string) => num.includes(searchTerm.trim()));

      const gameOk = gameFilter === 'ALL' || item.gameType === gameFilter;

      return termOk && gameOk;
    });
  }, [dictionaryRows, searchTerm, gameFilter]);
      ...
      <section style={{ padding: '32px', display: 'grid', gap: '24px' }}>
        <section className="journal-card">
          <div className="page-header">
            ...
            <p>
              Searchable, auto-built from uploaded dreams and parsed mappings, with single and batch manual add support.
            </p>
          </div>
        </section>
        ...
        <section className="journal-card-flat">
          ...
          >
            <div>
              <label className="journal-label">Search Term or Number</label>
              <div style={{ position: 'relative' }}>
                ...
                <input
                  ...
                  placeholder="cat, baby, 802..."
                />
              </div>
            </div>

            <div>
              <label className="journal-label">Game Filter</label>
              <select
                className="journal-select"
                value={gameFilter}
                onChange={e => setGameFilter(e.target.value as 'ALL' | 'cash3' | 'cash4' | 'mixed')}
              >
            ...
            <div className="journal-card-flat">
              <div className="journal-label">Visible Terms</div>
        ...
        {pageLoading ? (
          ...
          <section className="journal-card">
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px', color: 'var(--deep-plum)' }}>
              <Sparkles size={18} />
              <strong>No dictionary terms in this view</strong>
            </div>
            <p style={{ margin: 0, color: 'var(--ink-light)' }}>
              Adjust your filters or add more term-number entries.
            </p>
          </section>
        ) : (
...
export default function FellBeforePage() {
  ...
  const groupedItems = useMemo(() => {
    ...
    const dreamerGroups = new Map<string, any[]>();

    for (const item of subset) {
      if (!dreamerGroups.has(item.dreamerName)) {
        dreamerGroups.set(item.dreamerName, []);
      }
      dreamerGroups.get(item.dreamerName)!.push(item);
    }

    return Array.from(dreamerGroups.entries())
      .map(([dreamerName, rows]) => {
        const termGroups = new Map<string, any[]>();

        for (const row of rows) {
          if (!termGroups.has(row.termLabel)) {
            termGroups.set(row.termLabel, []);
          }
          termGroups.get(row.termLabel)!.push(row);
        }

        const groupedTerms = Array.from(termGroups.entries())
          .map(([termLabel, termRows]) => ({
            termLabel,
            rows: [...termRows].sort((a: any, b: any) => {
              if ((a.hitCount || 0) !== (b.hitCount || 0)) return (b.hitCount || 0) - (a.hitCount || 0);
              if (a.state !== b.state) return a.state.localeCompare(b.state);
              return a.number.localeCompare(b.number);
            }),
            totalHits: termRows.reduce((sum: number, row: any) => sum + (row.hitCount || 0), 0),
          }))
          .sort((a, b) => {
            ...
            {groupedItems.map(group => (
              <article key={group.dreamerName} className="journal-card">
                ...
                <div style={{ display: 'grid', gap: '18px' }}>
                  {group.groupedTerms.map((termGroup: any) => (
                    <div key={`${group.dreamerName}-${termGroup.termLabel}`} className="journal-card-flat">
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: '12px' }}>
                        <div>
                          <div style={{ fontWeight: 700, color: 'var(--deep-plum)', fontSize: '18px' }}>
                            {termGroup.termLabel}
                          </div>
                          <div style={{ color: 'var(--ink-light)', fontSize: '14px', marginTop: '4px' }}>
                            Total hits for term: {termGroup.totalHits}
                          </div>
                        </div>
                      </div>

                      <div style={{ display: 'grid', gap: '12px' }}>
                        {termGroup.rows.map((item: any, index: number) => (
                          <div key={item.id} className="journal-card-flat" style={{ background: 'rgba(255,255,255,0.72)' }}>
